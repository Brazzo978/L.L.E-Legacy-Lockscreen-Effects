...
- [x] Add Tablet mode detection/toggle next to Fold mode in setup and advanced
  settings.
- [x] Extend the display-profile resolver without breaking the existing
  `single`, `cover` and `main` preference/cache schema.
- [x] Keep separate automatic captures, imported wallpapers, timestamps,
  refresh tokens, dimensions and memory-cache identity for portrait and
  landscape.
- [x] Switch profiles on an actual orientation/display change and invalidate
  only the renderer source that belongs to the old orientation.
- [x] Load the correct cached colormap before the first visible effect frame
  when possible.
- [x] Provide independent capture/import/preview status for both tablet
  orientations in the wizard and control UI.
- [x] Preserve existing phone data as `single`; do not duplicate or erase it
  during migration.
- [x] Keep independent portrait and landscape touch boxes. Physical validation
  confirmed that reusing/scaling one box across tablet orientations is not
  correct; an unconfigured orientation uses the small safety box.
- [x] Include active mode, resolved orientation profile, bitmap dimensions and
  source path type (automatic/imported, never the private path itself) in the
  debug report.

### Implementation validation (Galaxy Tab A11+, Android 16)

- [x] Runtime rotation resolves `tablet_portrait` at `1200x1920` and
  `tablet_landscape` at `1920x1200`.
- [x] Service recreation loads two distinct automatic caches from disk at the
  matching dimensions; the opposite-orientation cache is never migrated or
  used as a fallback.
