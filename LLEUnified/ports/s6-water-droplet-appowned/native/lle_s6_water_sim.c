    ...
    for (particle_a_index = 0u; particle_a_index < group_a->count;
      ...
      for (group_b_index = 0u; group_b_index < LLE_S6_GROUP_CAPACITY;
        ...
        for (particle_b_index = 0u; particle_b_index < group_b->count;
          ...
          b->x += LLE_S6_STOCK_DT * impulse_x;
          b->y += LLE_S6_STOCK_DT * impulse_y;
        }
      }
      a->velocity_x += self_x;
      a->velocity_y += self_y;
      a->x += LLE_S6_STOCK_DT * self_x;
      a->y += LLE_S6_STOCK_DT * self_y;
    }
  }
}
...
static void s6_advance_domain(LleS6WaterSim *sim, bool primary) {
  size_t step;
  for (step = 0u; step < LLE_S6_WATER_SOLVER_STEPS_PER_TICK; ++step) {
    size_t group_index;
    const float pixels_per_world = s6_pixels_per_world(sim);
    for (group_index = 0u; group_index < LLE_S6_GROUP_CAPACITY;
         ++group_index) {
    ...
    s6_apply_sph_forces(sim, primary);
    for (group_index = 0u; group_index < LLE_S6_GROUP_CAPACITY;
         ++group_index) {
      LleS6Group *group = &sim->groups[group_index];
      size_t particle_index;
      ...
      for (particle_index = 0u; particle_index < group->count;
        ...
        particle->x += particle->velocity_x * LLE_S6_STOCK_DT;
        particle->y += particle->velocity_y * LLE_S6_STOCK_DT;
 ...
 */
static void s6_apply_wall_fields(LleS6WaterSim *sim) {
  size_t group_index;
  const bool tablet =
  ...
  for (group_index = 0u; group_index < LLE_S6_GROUP_CAPACITY;
    ...
    const float hard_step =
        0.35f * project_scale *
        (short_release ? 0.004166667f : 0.015f) *
        s6_pixels_per_world(sim);
    size_t particle_index;
    ...
    for (particle_index = 0u; particle_index < group->count;
      ...
      if (hard_y) {
        ...
        if (broad) {
          ...
          s6_nudge_primary_neighbours(
              sim, particle, direction_y * 0.35f * 0.0001f *
                                 s6_pixels_per_world(sim),
              false);
        }
      }
...
static void s6_grow_particle(const LleS6WaterSim *sim,
  ...
  particle->smoothing_radius +=
      (target_radius - particle->smoothing_radius) * curve;
  particle->rest_density +=
      (target_density - particle->rest_density) * curve;
  particle->pressure += (0.4f - particle->pressure) * curve;
  particle->near_pressure += (0.4f - particle->near_pressure) * curve;
}
  ...
  if (group->age >= 1.0f) {
    ...
    for (index = 0u; index < group->count; ++index) {
      ...
      if (particle->phase < 0.5f) {
        mature = false;
        s6_grow_particle(sim, particle, 0.021666666f);
        particle->extra_scale += 0.0025f;
      }
      {
        ...
        if (sqrtf(dx * dx + dy * dy) > sim->width * 0.16f) {
          ...
          particle->velocity_y *= velocity_scale;
        }
      }
    }
    if (mature) {
...
static void s6_advance_group(LleS6WaterSim *sim, LleS6Group *group) {
  ...
  if (group->phase == LLE_S6_GROUP_AFFORDANCE) {
    s6_advance_affordance(sim, group);
    return;
  }
    ...
    for (index = 0u; index < group->count; ++index) {
      ...
      if (group->age >= 0.3f) {
        ...
        if (below || particle->phase < 0.35f) {
          s6_grow_particle(sim, particle, LLE_S6_STOCK_DT);
          particle->released_phase = particle->phase;
  ...
  if (group->phase == LLE_S6_GROUP_RELEASE_EDGE) {
    ...
    for (index = 0u; index < group->count; ++index) {
      ...
      if (particle->active && particle->phase < 0.4f) {
        s6_grow_particle(sim, particle, 0.008f);
      }
    }
    group->age += 1.0f;
  ...
  if (group->phase == LLE_S6_GROUP_TOUCH) {
    ...
    for (index = 0u; index < group->count; ++index) {
      ...
      float dx;
      float dy;
      s6_grow_particle(sim, particle, 0.024166668f);
      if (particle->phase >= 1.0f) {
        if (sim->slow_touch) {
          particle->extra_scale =
              s6_min(particle->extra_scale + 0.0016666668f, 1.19f);
        } else {
          particle->extra_scale =
              s6_clamp(particle->extra_scale * 0.985f - 0.004f,
                        0.0f, 1.2f);
        }
      }
...
void lle_s6_water_sim_tick(LleS6WaterSim *sim) {
  ...
  if (!sim->unlocking) {
    ...
    sim->background_center_x +=
        (sim->touch_x - sim->background_center_x) * 0.05f;
    sim->background_center_y +=
        (sim->touch_y - sim->background_center_y) * 0.05f;
    (void)half_width;
    (void)half_height;
    if (sim->background_progress < 1.0f) {
      sim->background_progress =
          s6_min(sim->background_progress + 0.01325f, 1.0f);
    }
  } else {
    const float progress = sim->unlock_progress;
    if (sim->particle_size < 2000.0f) {
      sim->particle_size *= sim->unlock_from_edge ? 1.1f : 1.08f;
    }
    ...
    if (sim->unlock_delay_frames < 1) {
      ...
      if (progress < 1.0f) {
        sim->unlock_progress = s6_min(progress + 0.05f, 1.0f);
      } else {
  ...
  s6_advance_domain(sim, true);
  s6_advance_domain(sim, false);
