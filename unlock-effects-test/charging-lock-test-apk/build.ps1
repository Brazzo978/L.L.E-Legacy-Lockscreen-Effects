...
$unsigned = Join-Path $out "ChargingLockTest-unsigned.apk"
$aligned = Join-Path $out "ChargingLockTest-aligned.apk"
$signed = Join-Path $out "ChargingLockTest-debug.apk"
...
Run (Join-Path $buildTools "zipalign.exe") @("-f", "4", $aligned, (Join-Path $out "ChargingLockTest-zipaligned.apk"))
Run (Join-Path $buildTools "apksigner.bat") @("sign", "--ks", $keystore, "--ks-pass", "pass:android", "--key-pass", "pass:android", "--out", $signed, (Join-Path $out "ChargingLockTest-zipaligned.apk"))
